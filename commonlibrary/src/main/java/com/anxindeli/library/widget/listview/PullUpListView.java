...
/**
 * 上拉加载更多的ListView
 * Created by Adam_Lee on 2015/9/28.
 */
public class PullUpListView extends ListView implements AbsListView.OnScrollListener {
    ...
    private boolean mEnablePullLoad = false; //默认不可上拉, 此处避免最后点击footer出错
    ...
    public void setPullLoadEnable(boolean enable) {
        ...
        if (!mEnablePullLoad) {
            ...
            mPullLoading = false;
            mFooterView.show();
            mFooterView.setState(XListViewFooter.STATE_NORMAL, R.string.xlistview_footer_hint_normal);
    ...
    private void updateFooterHeight(float delta) {
        ...
        if (mEnablePullLoad && !mPullLoading) {
            if (height > PULL_LOAD_MORE_DELTA) { // height enough to invoke load
                // more.
                mFooterView.setState(XListViewFooter.STATE_READY, R.string.xlistview_footer_hint_ready);
            } else {
                mFooterView.setState(XListViewFooter.STATE_NORMAL, R.string.xlistview_footer_hint_normal);
            }
        }
    ...
    private void startLoadMore() {
        mPullLoading = true;
        mFooterView.setState(XListViewFooter.STATE_LOADING, 0);
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            if (mScrollBack == SCROLL_BACK_FOOTER) {
    ...
    /**
     * 上拉加载更多时结果提示（无网络、加载失败、没更多数据等）
     * @param textId
     */
    public void setFooterHint(int textId) {
        if (mPullLoading == true) {
            mPullLoading = false;
            mFooterView.setState(XListViewFooter.STATE_HINT, textId);
        }
    }
